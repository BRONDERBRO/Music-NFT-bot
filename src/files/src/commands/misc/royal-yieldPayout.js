...
const calculateYearlyRoyalties = require('../../utils/calculateYearlyRoyalties');
...
module.exports = {
    name: 'royal-yield-payout',
    description: 'Shows APR and floor for "Buy Now" values for Royal Songs taking into account previous payouts',
    // devOnly: Boolean,
    // testOnly: Boolean,
    // options: Object[],
    // deleted: Boolean,
    ...
    callback: async (client, interaction) => {
        ...
        //Get payout data from json file
        const payoutData = readJsonFile('src/files/payoutRoyal.json');
        ...
        const {
           ...
           for (const tier of fetchedRoyal.data.edition.tiers) {
                ...
                const { yearlyRoyalties, royaltyDate } = calculateYearlyRoyalties(payoutData.data, collectionName, collectionTier);
                ...
                */
                ...
                if (collectionRoyalties && floorPrice > 0) {
                    ...
                    if(yearlyRoyalties !== null){
                        expectedYield = yearlyRoyalties / floorPrice * 100
                    }
                    else{
        ...
        const embedDescription = 'Calculated yield of Royal songs: (yield % - $ floor - Royalties Paid)'
        ...
        for (let k = 0; k < yieldResultsLength; ++k) {
            ...
            const { name, tier, yield, floor, url, royaltyPaid } = yieldResults[k];
            const fieldName = `${name} - ${tier}`;
            const fieldValue = `[${yield}% - $ ${floor} - ${royaltyPaid}](${url})`;
